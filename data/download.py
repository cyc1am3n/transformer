...
if __name__ == '__main__':
    ...
    for x in [SRC, TGT]:
        with open(f'./ORIG/fr-en/{TRAIN_SET}{x}', 'r', encoding='UTF8') as f:
            b = f.read()
        with open(f'./PREP/train.{SRC}-{TGT}.{x}', 'w', encoding='UTF8') as f:
            f.write(re.sub(train_p, '', b))
            
    print('pre-processing valid data..')
    for x in [SRC, TGT]:
        text = []
        with open(f'./PREP/valid.{SRC}-{TGT}.{x}', 'w', encoding='UTF8') as f:        
    ...
    for x in [SRC, TGT]:
        text = []
        with open(f'./PREP/test.{SRC}-{TGT}.{x}', 'w', encoding='UTF8') as f:
